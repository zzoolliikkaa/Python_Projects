...
def upload_json_file(filename: str = None) -> dict:
    """
...
with st.sidebar:
    ...
    col = st.container()
    col.button("📋  List", use_container_width=True, on_click=set_mode, args=("List",))
    col.button(
        "🔎  Search", use_container_width=True, on_click=set_mode, args=("Search",)
    )
    col.button("📝  Add", use_container_width=True, on_click=set_mode, args=("Add",))
    col.button(
    ...
    if st.button("⬇️  Save phonebook to file", use_container_width=True):
        try:
            with open(PHONEBOOK_JSON_FILENAME, "w", encoding="utf-8") as f:
                json.dump(st.session_state.phonebook, f, ensure_ascii=False, indent=2)
...
if mode == "List":
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Phone_number": st.column_config.TextColumn(help="e.g. +1 202 555 0123"),
